...
:args_done
if not defined SOURCE (
  echo Kullanım: run_uxb_ast.bat [--alias-layer] uxb\tests\basicCodeTests\42_uxb_native_console_codegen_smoke.bas
  exit /b 1
)
...
uxb\compiler\wrappers\uxb_main_wrapper_64.exe "%EFFECTIVE_SOURCE%" --execmem --interpreter-backend AST --console-mode direct
exit /b %ERRORLEVEL%

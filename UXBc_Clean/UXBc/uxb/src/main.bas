...
#include "codegen/x64/mir_x64_field_synthetic.fbs"
#include "codegen/x64/mir_x64_keyword_surface.fbs"
#include "codegen/x64/mir_x64_capability.fbs"
#include "codegen/x64/mir_x64_codegen.fbs"
#include "codegen/x64/x64_codegen_policy.fbs"
#include once "codegen/x64/x64_extfp_call_emit.fbs"
...
Private Function UXBSourceFileExtension(ByRef pathText As String) As String
    Dim As String p
...
Private Function UXBValidateSourcePathPolicy(ByRef sourcePath As String, ByRef errText As String) As Integer
    Dim As String ext
    ext = UXBSourceFileExtension(sourcePath)

    Select Case ext
    Case ".bas", ".uxm"
        Return 1
    Case ".uxmh"
        errText = "TR: .uxmh baslik dosyasi dogrudan calistirilamaz; include edilmelidir | EN: .uxmh header files cannot be executed directly; they must be included"
        Return 0
    Case Else
        errText = "TR: Desteklenmeyen kaynak dosya uzantisi: " & ext & " (.bas/.uxm bekleniyor) | EN: Unsupported source file extension: " & ext & " (expected .bas/.uxm)"
        Return 0
    End Select
End Function

Private Function UXBValidateArtifactPathPolicy(ByRef artifactPath As String, ByRef errText As String) As Integer
    Dim As String p
    p = Trim(artifactPath)
    If p = "" Then Return 1

    If UXBProjectOutputIsSourcePollution(p) <> 0 Then
        errText = "TR: Cikti dosyasi uxb/src altini kirletemez: " & p & " | EN: Output artifact cannot pollute uxb/src: " & p
        Return 0
    End If

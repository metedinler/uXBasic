...
Private Sub Main()
    ...
    ok And= RTAssertEq(VMemPeekD(7812), 1432778632, "INPUTF file second value")
    ...
    If RTParseExpectFail("INPUTF 1, 9", "INPUT#: targets must be IDENT", negErr) = 0 Then
        Print "FAIL parse-neg INPUTF target kind | "; negErr
        ok = 0
    End If

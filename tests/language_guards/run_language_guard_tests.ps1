[CmdletBinding()]
param([string]$Uxb = ".\bin\uxb.exe", [string]$TestRoot = ".\tests\language_guards")
# Dil korumalari (rezerve adlar vb.) -- derleme zamani hatalari: her test AST, canonical-MIR, native x64 (bld) ve legacy AST-x64'te
# calistirilir; ciktilar expected/<ad>.txt ile birebir karsilastirilir.
$ErrorActionPreference = "Continue"
# INFO satirlari ve AST'nin FFI ilke uyarilari (stderr, REPORT_ONLY) sonuca dahil degil
function Norm([string]$s) { (($s -replace "`r","") -split "`n" | Where-Object { $_ -notmatch '^INFO|^WARN: FFI|^\s*$' }) -join "`n" }
$pass = 0; $fail = 0
foreach ($f in Get-ChildItem $TestRoot -Filter *.uxb | Sort-Object Name) {
  ...
  foreach ($mode in @(@("x64", @()), @("AST-x64", @("--x64-mode","AST")))) {
    ...
    $o = (& $Uxb bld $f.FullName @($mode[1]) -o "out\lg_$($mode[0]).exe" 2>&1) -join "`n"

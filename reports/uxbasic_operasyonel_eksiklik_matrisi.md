...
| Calling convention: stdcall/cdecl secimi | OK | OK | OK | OK | OK | Parser+runtime token destegi aktif (CDECL/STDCALL). Win64 runtime invoke lane'i I32 disina genisletildi (U64/F64/PTR/STRPTR/BYREF/BYVAL, 0..4 arg). x86 native lane raporu host-bagimsiz fallback proof ile tekrar-edilebilir hale getirildi (`tools/run_ffi_conv3_native_lanes.ps1`, `tests/run_ffi_x86_resolver_cleanup_proof.bas`). | FFI-CONV-1 |
...
| x86 stdcall/cdecl ayristirma | OK | OK | OK | OK | OK | x86 lane kapanisi: plan+stub+resolver artifact zinciri, cleanup contract gate'i ve symptr write-through dogrulamasi tamamlandi. Native probe adimi hostta `__FB_32BIT__` SKIP donerse hostless fallback proof zorunlu calisiyor (`tests/run_ffi_x86_resolver_cleanup_proof_64.exe`) ve lane PASS/PASS kapanir. Kanit: `tools/run_ffi_conv3_native_lanes.ps1`, `reports/ffi_conv3_native_lanes_report.md`, `tests/run_ffi_x86_call_backend.bas`, `tests/run_ffi_x86_resolver_cleanup_proof.bas`. | FFI-CONV-3 |
...
| Regression gate (interp vs compiled parity) | OK | OK | OK | OK | OK | ERR parity kapisi script ile zorunlu hale getirildi; lane raporu uretiliyor ve Faz A gate icine baglandi (`tools/run_err_codegen_parity_gate.ps1`, `reports/err_codegen_parity_gate_report.md`, `tools/run_faz_a_gate.ps1`). | CG-QA |
...
| FFI | CALL(DLL), IMPORT, INLINE | OK | OK | OK | OK | Attestation hash/signer extraction fail-closed yoluna tasindi (gercek DLL hash ve signer cikarma, fixture adlarinda deterministic test istisnalari). x86 native lane hostless fallback proof ile tekrar-edilebilir kapanisa alindi. Kanit: `src/runtime/ffi_signer.fbs`, `tests/run_call_exec.bas`, `tools/run_ffi_conv3_native_lanes.ps1`, `reports/ffi_conv3_native_lanes_report.md`. |
| Hata Yonetimi | TRY/CATCH/FINALLY, THROW, ASSERT, ERROR objesi | OK | OK | OK | OK | ERR semantic/mir/backend/parity lane'leri zorunlu parity gate altinda raporlanir hale getirildi (`tools/run_err_codegen_parity_gate.ps1`). |

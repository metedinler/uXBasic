...
| TRY/CATCH/FINALLY/END TRY | OK | OK | OK | OK | OK | ERR parser+semantic+runtime kapanisi tamamlandi: nested TRY rethrow unwind + FINALLY garanti icra testle dogrulandi; CATCH/FINALLY sira/sayi guard fail-fast aktif. Kanit: `tests/run_err_try_throw_assert_exec.bas`, `tests/run_err_semantic_pass.bas`. | ERR-1 |
| THROW | OK | OK | OK | OK | OK | THROW sozlesmesi kapandi: `THROW code[, message[, detail]]` parser/semantic/runtime akisi aktif, yakalanmayan THROW deterministic fail-fast (`exec: unhandled THROW code=... message=... detail=...`). Kanit: `tests/run_err_try_throw_assert_exec.bas`, `tests/run_err_semantic_pass.bas`. | ERR-1 |
| ASSERT | OK | OK | OK | OK | OK | ASSERT sozlesmesi kapandi: `ASSERT expr[, message]` debug modda THROW uretir, release modda no-op policy aktif; semantic arguman guard fail-fast. Kanit: `tests/run_err_try_throw_assert_exec.bas`, `tests/run_err_semantic_pass.bas`. | ERR-2 |
...
| x86 stdcall/cdecl ayristirma | OK | OK | OK | OK | OK | x86 lane kapanisi tamamlandi: plan+stub + resolver artifact (`dist/interop/ffi_call_x86_resolver.csv`) uretiliyor; runtime resolver enforce/report + symbol binding cache/symptr map aktif (`ExecDebugGetFfiX86ResolvedCount`, `ExecDebugGetFfiX86SymptrMapCount`). Invoke-proof yolu I32 imza icin 0..4 arg pointer call'a genisletildi ve cleanup byte sayaçlari eklendi (`ExecDebugGetFfiX86InvokeCount`, `ExecDebugGetFfiX86CallerCleanupBytes`, `ExecDebugGetFfiX86CalleeCleanupBytes`). Symptr write-through gozlemlenebilirligi aktif (`ExecDebugGetFfiX86SymptrWriteCount`, `ExecDebugGetFfiX86SymptrLabelByStubId`, `ExecDebugGetFfiX86SymptrProcAddrByStubId`) ve resolver cleanup proof PASS. Native lane probe seti (`tests/probes/run_ffi_x86_native_cleanup_probe.bas`, `tests/probes/run_ffi_x86_native_symptr_patch_probe.bas`) host uzerinde PASS/PASS dogrulandi (`reports/ffi_conv3_native_lanes_report.md`): `native_cleanup` PASS, `native_symptr_patch` PASS (cmd fallback). Native lane raporu `tools/run_ffi_conv3_native_lanes.ps1` icinde run-launch fallback ve timeout guard ile sertlestirildi; cleanup probe 32-bit hedef tespiti `__FB_WIN32__` + `Not __FB_WIN64__` ile deterministic hale getirildi. Cleanup contract gate'i aktif (`ExecX86FfiValidateCleanupContract`). Kanit: `tests/run_ffi_x86_call_backend.bas`, `tests/run_ffi_x86_resolver_exec_ast.bas`, `tests/run_ffi_x86_resolver_cleanup_proof.bas`, `tests/run_call_exec.bas`, `reports/ffi_conv3_native_lanes_report.md`. | FFI-CONV-3 |
| Ilk resmi DLL: uXStat | OK | OK | OK | OK | OK | `extras/uxstat` altinda minimum C ABI cekirdegi (`uxstat_ping`, `uxstat_add_i32`) eklendi ve x64 CALL(DLL) smoke lane'i ile dogrulandi. Kanit: `tests/run_uxstat_smoke_exec_ast.bas`, `tools/run_uxstat_smoke_gate.ps1`. | UXSTAT-0 |
...
| CALL [register] / stack arg passing | OK | OK | OK | OK | OK | x64 lane'de symbol resolution baglama kapanisi tamamlandi: resolver artifact (`dist/interop/ffi_call_x64_resolver.csv`) emit + runtime resolver cache/symptr map/invoke-proof aktif. Kanit: `tests/run_ffi_x64_resolver_exec_ast.bas` (`ExecDebugGetFfiX64ResolvedCount`, `ExecDebugGetFfiX64SymptrMapCount`, `ExecDebugGetFfiX64InvokeCount`). | CG-3 |
| Win64 ABI (shadow space + alignment) | OK | OK | OK | OK | OK | Win64 ABI reserve/alignment yoluna ek olarak runtime x64 resolver->invoke zinciri enforce/report modlarinda dogrulandi; eksik resolver fail-fast davranisi ve invoke sayaçlari testte kanitlandi. Kanit: `tests/run_ffi_x64_resolver_exec_ast.bas`, `tests/run_uxstat_smoke_exec_ast.bas`. | CG-3 |
| Regression gate (interp vs compiled parity) | OK | OK | OK | OK | OK | ERR codegen parity lane aktif: `tests/run_err_codegen_parity_gate.bas` ile interpreter cikisi + MIR/hook/asm yapisal sozlesme ayni kaynakta dogrulaniyor; compiled interop kaniti `tools/run_err_codegen_parity_gate.ps1` ile `src/main_64.exe --interop` uzerinden gate'e baglandi. Task: `validate_err_codegen_parity_gate_ps`. | CG-QA |
| TRY/CATCH unwinding emit (label table + finally trampoline) | OK | OK | OK | OK | OK | TRY/CATCH/FINALLY codegen kapanisi: hook CSV'de register/unregister + edge trampoline satirlari (`REGISTER_HANDLER_REGION`, `UNREGISTER_HANDLER_REGION`, `JMP_TRAMPOLINE`) ve FINALLY label emit'i (`LABEL_FINALLY_STUB`) aktif. Asm tarafinda stack tabanli handler lifecycle (`__uxb_err_handler_top/capacity/table`) + LIFO dispatch calisiyor. Kanit: `tests/run_err_backend_hooks.bas`, `tests/run_err_backend_artifacts.bas`. | ERR-CG-1 |
| THROW emit (error object materialization + jump to handler) | OK | OK | OK | OK | OK | THROW codegen kapanisi: ABI layout/field satirlari (`ERR_THROW_LAYOUT`, `ERR_THROW_FIELD_CODE`, `ERR_THROW_FIELD_MESSAGE`, `ERR_THROW_FIELD_DETAIL`) uretiliyor; asm'de pending payload staging (`__uxb_err_pending_*`) -> error object materialization (`__uxb_err_obj_*`) koprusu ve getter sembolleri (`__uxb_err_get_throw_object`, `__uxb_err_get_throw_code_ptr`, `__uxb_err_get_throw_message_ptr`, `__uxb_err_get_throw_detail_ptr`) aktif. Kanit: `tests/run_err_mir_lowering.bas`, `tests/run_err_backend_hooks.bas`, `tests/run_err_backend_artifacts.bas`. | ERR-CG-2 |
| ASSERT emit (debug/release policy) | OK | OK | OK | OK | OK | ASSERT codegen kapanisi: policy hook (`CHECK_ASSERT_POLICY_STUB`) ve assert mode set/get bridge (`__uxb_err_set_assert_release_mode`, `__uxb_err_get_assert_release_mode`, `__uxb_err_assert_release_mode`) emit + artifact testleri ile dogrulandi. Kanit: `tests/run_err_backend_hooks.bas`, `tests/run_err_backend_artifacts.bas`. | ERR-CG-3 |
...
| Hata Yonetimi | TRY/CATCH/FINALLY, THROW, ASSERT, ERROR objesi | OK | OK | OK | OK | ERR lane kapanisi: parser+semantic+runtime + codegen emit tamamlandi; parity gate aktif (`tests/run_err_codegen_parity_gate.bas`, `tools/run_err_codegen_parity_gate.ps1`). |

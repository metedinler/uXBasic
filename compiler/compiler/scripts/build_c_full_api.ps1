...
function Test-SystemDll([string]$Name) {
    ...
    if ($upper -match "^(KERNEL32|USER32|ADVAPI32|SHELL32|OLE32|OLEAUT32|GDI32|WS2_32|CRYPT32|BCRYPT|NTDLL|RPCRT4|COMDLG32|COMCTL32|SHLWAPI|VERSION|WINMM|IPHLPAPI|NORMALIZ|SECUR32|UCRTBASE|MSVCRT)\.DLL$") {
        return $true
    }
...
function Assert-Pe64([string]$DllPath) {
    ...
    if ($LASTEXITCODE -ne 0 -or $text -notmatch "pei-x86-64") {

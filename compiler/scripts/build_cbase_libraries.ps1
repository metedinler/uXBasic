...
function Invoke-GccBuild {
    ...
    try {
        $buildOutput = & $script:Gcc @args 2>&1
        $buildExitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $oldErrorActionPreference
    }
    $buildOutput | Tee-Object -FilePath (
...
function Test-IsSystemDll([string]$Name) {
    ...
    if ($upper -match "^(KERNEL32|USER32|ADVAPI32|SHELL32|OLE32|OLEAUT32|GDI32|WS2_32|WLDAP32|DNSAPI|CRYPT32|BCRYPT|NTDLL|RPCRT4|COMDLG32|COMCTL32|SHLWAPI|VERSION|WINMM|IPHLPAPI|NORMALIZ|SECUR32|UCRTBASE|VCRUNTIME140|MSVCP140)\.DLL$") {
        return $true
    }
...
function Fetch-Yyjson {
    $version = "0.12.0"
    $expected = "b16246f617b2a136c78d73e5e2647c6f1de1313e46678062985bdcf1f40bb75d"
...
if ($InstallDeps) {
    ...
    $ErrorActionPreference = "Continue"
    try {
        $pacmanOutput = & $Pacman -S --needed --noconfirm @packages 2>&1
        $pacmanExitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $oldErrorActionPreference
    }
    $pacmanOutput | Tee-Object -FilePath (Join-Path $Session "pacman.log")
